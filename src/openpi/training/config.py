...
import openpi.models.pi0_pcd as pi0_pcd
import openpi.models.pi0_fast as pi0_fast
import openpi.models.tokenizer as _tokenizer
import openpi.policies.aloha_policy as aloha_policy
import openpi.policies.droid_policy as droid_policy
import openpi.policies.libero_policy as libero_policy
import openpi.policies.pcd_policy as pcd_policy
...
@dataclasses.dataclass(frozen=True)
class PCDDataConfig(DataConfigFactory):
    @override
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        repack_transform = _transforms.Group(
            inputs=[
        ...
        data_transforms = _transforms.Group(
            inputs=[pcd_policy.PCDInputs(action_dim=model_config.action_dim, model_type=model_config.model_type)],
            outputs=[pcd_policy.PCDOutputs()],
        )
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        batch_size=32,
        num_workers=8,
        model=pi0_pcd.Pi0PCDConfig(action_horizon=10),
        data=PCDDataConfig(
            repo_id="username/data_name",
            base_config=DataConfig(
                local_files_only=True,  # Set to True for local-only datasets.
